...
class C(BaseConstants):
    NAME_IN_URL = 'SimpleRanking_tiesincentives'
...
class Player(BasePlayer):
    ...
    pref1_1i = models.StringField()
    pref1_2i = models.StringField(blank=True)
    pref1_3i = models.StringField(blank=True)
    pref2_1i = models.StringField(blank=True)
    pref2_2i = models.StringField(blank=True)
    pref3_1i = models.StringField(blank=True)
...
class Ranking1(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if len(li)==1:
            player.pref1_1i = li[0]
        elif len(li) == 2:
            player.pref1_1i = li[0]
            player.pref1_2i = li[1]
        elif len(li) == 3:
            player.pref1_1i = li[0]
            player.pref1_2i = li[1]
            player.pref1_3i = li[2]
            player.imp = random.choices([player.pref1_1i,player.pref1_2i, player.pref1_3i,"'Watch the video first and then choose between A and B.'"], weights=(20,20,20,40),k=1)[0]
        else:
            player.pref1_1i = "Error"
        player.length2 = len(list(player.finalRanking2.split(",")))
        li2 = list(player.finalRanking2.split(","))
        if len(li2) == 1:
            player.pref2_1i = li2[0]
        elif len(li2) == 2:
            player.pref2_1i = li2[0]
            player.pref2_2i = li2[1]
        else:
            player.pref2_1i = "Error"
        player.length3 = len(list(player.finalRanking3.split(",")))
        li3 = list(player.finalRanking3.split(","))
        if len(li3) == 1:
            player.pref3_1i = li3[0]
        else:
            player.pref3_1i = "Error"
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if player.WTA1 == "Yes":
            if len(li)==1 and len(li2)==1:
                player.pref1_1 = li2[0]
                player.pref2_1 = li[0]
                player.imp = random.choices([player.pref1_1 , "'Watch the video first and then choose between A and B.'"], weights=(40, 60),k=1)[0]
            elif len(li)==1 and len(li2)==2:
                player.pref1_1 = li2[0]
                player.pref1_2 = li2[1]
                player.pref2_1 = li[0]
                player.imp = random.choices([player.pref1_1,player.pref1_2, "'Watch the video first and then choose between A and B.'"], weights=(20,20, 60),k=1)[0]
            elif len(li)==2 and len(li2)==2:
                player.pref1_1 = li2[0]
                player.pref1_2 = li2[1]
                player.pref2_1 = li[0]
                player.pref2_2 = li[1]
                player.imp = random.choices([player.pref1_1, player.pref1_2, "'Watch the video first and then choose between A and B.'"], weights=(30,30, 40),k=1)[0]
            elif len(li) == 2 and len(li2) == 1:
                player.pref1_1 = li2[0]
                player.pref2_1 = li[0]
                player.pref2_2 = li[1]
                player.imp = random.choices([player.pref1_1, "'Watch the video first and then choose between A and B.'"], weights=(60, 40),k=1)[0]
            elif len(li) == 2 and len(li2) == 1:
                player.imp = random.choices([player.pref1_1, player.pref1_2, player.pref1_3, "'Watch the video first and then choose between A and B.'"],
                                        weights=(20,20,20, 40), k=1)[0]
        elif player.WTA1 == "No":
            if len(li)==1:
                player.imp = random.choices([player.pref1_1, "'Watch the video first and then choose between A and B.'"], weights=(60, 40),k=1)[0]
            elif len(li)==2:
...
page_sequence = [Part2_Instruction_Page, Ranking1,Ranking1_wta, Part3_Intro, Video_alert, Part3_Video, Hypo_choice, Hypo_choiceq, qa, survey2, Openq, Attention1, FailedAttention,EQ, Feedback, Back]
