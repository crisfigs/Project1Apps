...
class C(BaseConstants):
    NAME_IN_URL = 'RankingSimple_Incentives1'
...
class Ranking__(Page):
    ...
    def before_next_page(player, timeout_happened):
        if player.WTA1 == "No" and player.WTA2 == "No":
            #0.5*x, 0.3*x, 0.2*x, (1-x)
            player.imp = random.choices([player.pref1i,player.pref2i, player.pref3i,"'Watch the video first and then choose between A and B.'"], weights=(0.3,0.18,0.12,0.4), k=1)[0]
        elif player.WTA1 == "Yes" and player.WTA2 == "No":
            # 0.4*x, 0.4*x, 0.2*x, (1-x)
            player.imp = random.choices([player.pref1i,player.pref2i, player.pref3i,"'Watch the video first and then choose between A and B.'"], weights=(0.24,0.24,0.12,0.4), k=1)[0]
        elif player.WTA1 == "No" and player.WTA2 == "Yes":
            # 0.5*x, 0.25*x, 0.25*x, (1-x)
            player.imp = random.choices([player.pref1i,player.pref2i, player.pref3i,"'Watch the video first and then choose between A and B.'"], weights=(0.3,0.15,0.15,0.4), k=1)[0]
        elif player.WTA1 == "Yes" and player.WTA2 == "Yes":
            # 1/3*x, 1/3*x, 1/3*x, (1-x)
            player.imp = random.choices([player.pref1i,player.pref2i, player.pref3i,"'Watch the video first and then choose between A and B.'"], weights=(0.2,0.2,0.2,0.4), k=1)[0]
        else:
